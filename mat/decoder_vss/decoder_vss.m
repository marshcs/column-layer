% function [decode_bit] = decoder_vss(llr,base_matrix,PARA)
	...
	for iter = 1:max_iter
		...
		if(iter == 1)
			...
			d1_mem_llr = (0.5-llr)*2*strength;
			d1_mem_c2v = struct(	'min', max_abs * ones(blk_size,pcm_rown,min_num),		...
								'idx', max_col_cnt * ones(blk_size, pcm_rown,min_num),	...
								'global_sign', zeros(blk_size,pcm_rown)					...
				);
			d1_mem_v2c_sign 	= zeros(blk_size, pcm_rown, pcm_coln,'logical');
			d1_mem_app			= zeros(blk_size,pcm_coln,'logical');
			
			d2_mem_llr = (0.5-llr)*2*strength;
			d2_mem_c2v = struct(	'min', max_abs * ones(blk_size,pcm_rown,min_num),		...
								'idx', max_col_cnt * ones(blk_size, pcm_rown,min_num),	...
								'global_sign', zeros(blk_size,pcm_rown)					...
				);
			d2_mem_v2c_sign 	= zeros(blk_size, pcm_rown, pcm_coln,'logical');
			d2_mem_app			= zeros(blk_size,pcm_coln,'logical');
		...
		for col = 1:pcm_coln
			...
			old_app = mem_app(:,col);
			mem_app(:,col) = app;
			app_diff = xor(mem_app(:,col),old_app);
			for row = 1:pcm_rown
