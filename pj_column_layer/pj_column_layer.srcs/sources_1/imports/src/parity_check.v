...
assign	w_addrb_read = i_is_first_iter ? i_col_cnt_output : i_col_cnt_mem;
...
// ---------------------v checksum update v-----------------------------------
	...
	assign		o_parity_check_ok = & w_parity_check_ok;
	generate
...
// ---------------------v barrel shifter v------------------------------------
wire	[GF_SIZE_LOG2-1:0]		w_shift_offset		[0:PCM_ROWN];
wire	[BLK_SIZE-1:0]			w_app_diff_shift	[0:PCM_ROWN];

generate
	for(i=0; i< PCM_ROWN; i=i+1)	begin: Barrel_Shifter_Ter

		assign		w_shift_offset = i_shift_offset[GF_SIZE_LOG2*(i+1)-1: GF_SIZE_LOG2*i];
